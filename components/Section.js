...
export default function Section({
  ...
  if (projectInfo !== null) {
    return (
      <section className="max-w-screen-lg px-8 w-full">
        <div className="border-t border-base-black dark:border-base-200">
          <div className="sm:grid grid-cols-3 gap-6 mb-12 sm:mb-0">
            {projectInfo.map((project, index) => (
              <div key={index} className="flex flex-col col-span-1">
                <h2 className="font-sans text-base mt-4 mb-2 text-base-black dark:text-base-200">
                  {project.title}
                </h2>
                <h3 className="font-sans text-base mb-4 sm:mb-12 text-base-600 dark:text-base-500 text-pretty">
                  {project.description}
                </h3>
              </div>
            ))}
